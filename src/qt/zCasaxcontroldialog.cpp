...
#include "ui_zCasaxcontroldialog.h"
...
std::list<std::string> ZCasaxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCasaxControlDialog::listMints;

ZCasaxControlDialog::ZCasaxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCasaxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCasaxControlDialog::~ZCasaxControlDialog()
{
    delete ui;
}

void ZCasaxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCasaxControlDialog::updateList()
{
...
void ZCasaxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCasaxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCasax_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCasaxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCasaxControlDialog::GetSelectedMints()
{
...
void ZCasaxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
