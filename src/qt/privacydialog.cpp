                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCSX ought to be enough for anybody." - Bill Gates, 2017
    ui->zCSXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCSXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCSX->setEnabled(false);
        ui->pushButtonMintzCSX->setToolTip(tr("zCSX is currently disabled due to maintenance."));

        ui->pushButtonSpendzCSX->setEnabled(false);
        ui->pushButtonSpendzCSX->setToolTip(tr("zCSX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCSXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCSX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCSX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCSX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCSX in ") +
...
void PrivacyDialog::on_pushButtonSpendzCSX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCSX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCSX
        sendzCSX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCSX
    sendzCSX();
}

void PrivacyDialog::on_pushButtonZCasaxControl_clicked()
{
    ZCasaxControlDialog* zCasaxControl = new ZCasaxControlDialog(this);
    zCasaxControl->setModel(walletModel);
    zCasaxControl->exec();
}

void PrivacyDialog::setZCasaxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCasaxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCSX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Casax Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCSXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCSX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CSX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCSX</b>";
    ...
    // use mints from zCasax selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCasaxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCasaxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zCasax selector in case it was used
    ZCasaxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCasax Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Casax, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCasax Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCSX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCSX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCSX "));
    ui->labelzCSXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCSXSyncStatus->setVisible(fShow);
}
